...
const parentGdu = "gdu-1044b-mfd";

export function createGia63w2Computer(context: ComputerRuntimeContext): ComputerProgram {
  return {
    ...
    start() {
      context.log.info("No. 2 GIA 63W IAU computer online", {
        parentGdu,
      });
    },

    stop() {
      context.log.info("No. 2 GIA 63W IAU computer stopped");
    },
  };
}
...
function acknowledgeGduInput(context: ComputerRuntimeContext, payload: unknown): void {
  ...
  context.log.debug("Processed MFD GDU key input", {
    action: input.action,
